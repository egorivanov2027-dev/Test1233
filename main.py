...
def format_number(num) -> str:
    """Форматирует число из строки с пробелами (разделители тысяч)"""
    try:
        num_str = str(num).strip()
        
        # Убираем пробелы если уже отформатировано
        num_str = num_str.replace(' ', '')
        
        # Конвертируем научную нотацию или дробные числа в целое
        if 'e' in num_str.lower() or 'E' in num_str:
            from decimal import Decimal
            d = Decimal(num_str)
            num_str = format(d, 'f')
        
        # Убираем дробную часть если она .0
        if '.' in num_str:
            integer_part, frac_part = num_str.split('.', 1)
            if not frac_part or all(c == '0' for c in frac_part):
                num_str = integer_part
            else:
                num_str = integer_part  # для баланса всегда целое
        ...
        # Форматируем с пробелами каждые 3 цифры — БЕЗ обрезания, показываем полностью
        formatted = ''
...
def validate_bet(bet: str, balance: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        # Проверка на достаточность средств (ставить можно любую сумму до баланса)
        clean_balance = str(balance).replace(' ', '')
        if bet_decimal > Decimal(clean_balance):
...
class SQLiteDatabase:
    ...
    def update_balance(self, user_id: int, amount) -> Tuple[bool, str, str]:
        ...
        if amount_str.startswith('-'):
            ...
            if current < Decimal(amount_str[1:]):
        ...
        for attempt in range(3):
            try:
                ...
                try:
                    current_dec = Decimal(str(current_balance).replace(' ', '').split('.')[0])
                    amount_dec = Decimal(str(amount_str).replace(' ', '').split('.')[0])
...
async def mines_start(message: Message):
    ...
    if parts[1] == "фулл":
        bet = db.get_balance(user.id).replace(' ', '')
    else:
        bet = parts[1].replace(' ', '')
