...
def format_number(num) -> str:
    """Форматирует число с пробелами как разделителями тысяч. Всегда целое, никаких точек."""
    try:
        raw = str(num).strip().replace(' ', '').replace(',', '')
        if not raw or raw == '-':
            return '0'
        # Decimal корректно разбирает любой формат: int, float, научную нотацию, строки с точкой
        val = int(Decimal(raw).to_integral_value())
        # Встроенный форматтер: запятые → пробелы
...
def validate_bet(bet: str, balance: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        # Нормализуем баланс: убираем пробелы, дробную часть
        try:
            clean_balance = Decimal(str(balance).replace(' ', '').replace(',', ''))
            clean_balance_int = int(clean_balance.to_integral_value())
        except Exception:
            return False, "❌ Ошибка чтения баланса"
        
        if bet_decimal > clean_balance_int:
...
class SQLiteDatabase:
    ...
    def init_db(self):
        ...
        try:
            ...
            # Сначала проверим существующие таблицы и добавим недостающие колонки
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='mines_games'")
            if cursor.fetchone():
                # Таблица существует, проверим наличие колонки status
                cursor.execute("PRAGMA table_info(mines_games)")
                columns = [col[1] for col in cursor.fetchall()]
                if 'status' not in columns:
                    cursor.execute("ALTER TABLE mines_games ADD COLUMN status TEXT DEFAULT 'active'")
                    logger.info("✅ Добавлена колонка status в таблицу mines_games")
            
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='box_games'")
            if cursor.fetchone():
                cursor.execute("PRAGMA table_info(box_games)")
                columns = [col[1] for col in cursor.fetchall()]
                if 'status' not in columns:
                    cursor.execute("ALTER TABLE box_games ADD COLUMN status TEXT DEFAULT 'active'")
                    logger.info("✅ Добавлена колонка status в таблицу box_games")
            
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='secret_room_games'")
            if cursor.fetchone():
                cursor.execute("PRAGMA table_info(secret_room_games)")
                columns = [col[1] for col in cursor.fetchall()]
                if 'status' not in columns:
                    cursor.execute("ALTER TABLE secret_room_games ADD COLUMN status TEXT DEFAULT 'active'")
                    logger.info("✅ Добавлена колонка status в таблицу secret_room_games")
            ...
            # Миграция: нормализуем балансы с дробной частью или пробелами
            try:
                cursor.execute('SELECT user_id, balance FROM users')
                rows = cursor.fetchall()
                updated = 0
                for row in rows:
                    raw = str(row['balance']).strip()
                    if '.' in raw or ' ' in raw or (raw and not raw.lstrip('-').isdigit()):
                        try:
                            normalized = str(int(Decimal(raw.replace(' ', '')).to_integral_value()))
                            cursor.execute('UPDATE users SET balance = ? WHERE user_id = ?', (normalized, row['user_id']))
                            updated += 1
                        except Exception:
                            cursor.execute('UPDATE users SET balance = ? WHERE user_id = ?', ('0', row['user_id']))
                            updated += 1
                if updated:
                    conn.commit()
                    logger.info(f"🔧 Нормализовано {updated} балансов в БД")
            except Exception as e:
                logger.error(f"Ошибка миграции балансов: {e}")
        except Exception as e:
        ...
        for attempt in range(3):
            try:
                ...
                if not result:
                    ...
                    return '0'
                raw = str(result['balance']).strip().replace(' ', '')
                try:
                    return str(int(Decimal(raw).to_integral_value()))
                except Exception:
                    return '0'
            except sqlite3.OperationalError as e:
                if "locked" in str(e) and attempt < 2:
    ...
    def update_balance(self, user_id: int, amount) -> Tuple[bool, str, str]:
        ...
        # Проверка на отрицательный баланс (предварительная, точная проверка будет внутри транзакции)
        if amount_str.startswith('-'):
            raw_bal = self.get_balance(user_id)  # already clean int string after get_balance fix
            try:
                current = Decimal(raw_bal)
                if current < Decimal(amount_str[1:].replace(' ', '')):
                    logger.warning(f"Попытка уйти в минус: {user_id}, сумма {amount_str}")
                    return False, "❌ Недостаточно средств", raw_bal
            except Exception:
                pass
        ...
        for attempt in range(3):
            try:
                ...
                try:
                    ...
                    # Всегда целое — никаких дробей в балансе
                    if new_dec < 0:
                        return False, "❌ Недостаточно средств", str(int(current_dec.to_integral_value()))
                    new_balance = str(int(new_dec.to_integral_value()))
        ...
        for attempt in range(3):
            try:
                ...
                try:
                    ...
                    new_from = str(int((from_dec - amount_dec).to_integral_value()))
                    ...
                    new_to = str(int((to_dec + amount_dec).to_integral_value()))
                except Exception as e:
    ...
    def open_box(self, user_id: int, box_num: int) -> dict:
        ...
        try:
            ...
            if game['gold_box'] == box_num:
                ...
                # Атомарно меняем статус ДО начисления выигрыша
                cursor.execute(
                    'UPDATE box_games SET status = ? WHERE user_id = ? AND status = ?',
                    ('won', user_id, 'active')
                )
                conn.commit()
    ...
    def open_multiple_cases(self, user_id: int, case_type: str, count: int) -> dict:
        ...
        try:
            ...
            for i in range(count):
                ...
                conn_skin = None
                try:
                    conn_skin = self.get_connection()
                    cursor_skin = conn_skin.cursor()
                    cursor_skin.execute('''
                        INSERT INTO skins (user_id, name, rarity, price, case_type)
                        VALUES (?, ?, ?, ?, ?)
                    ''', (user_id, skin['name'], skin['rarity'], skin['price'], case_type))
    ...
    def open_cell(self, user_id: int, cell: int) -> dict:
        ...
        try:
            ...
            try:
                current_win = Decimal(str(game['current_win']).replace(' ', '').split('.')[0])
                multiplier = Decimal('1.2')
                new_win_dec = current_win * multiplier
                # Всегда округляем до целого чтобы не копить дробные цифры
                new_win = str(int(new_win_dec.to_integral_value()))
            except Exception as e:
                logger.error(f"Ошибка расчета выигрыша: {e}")
                new_win = str(game['current_win']).split('.')[0]
            ...
            if len(game['opened_cells']) == len(safe_cells):
                # Атомарно меняем статус ДО начисления выигрыша
                cursor.execute(
                    'UPDATE mines_games SET status = ? WHERE user_id = ? AND status = ?',
                    ('won', user_id, 'active')
                )
                conn.commit()
                if cursor.rowcount == 0:
    ...
    def take_win(self, user_id: int) -> dict:
        ...
        try:
            ...
            if not opened_cells:
                return {'status': 'error', 'message': 'Нельзя забрать без открытых клеток'}
        ...
        try:
            ...
            if door in game['monster_doors']:
                ...
                if game['treasures_found'] == 3:
                    # Атомарно меняем статус ДО начисления выигрыша
                    cursor.execute(
                        'UPDATE secret_room_games SET status = ? WHERE user_id = ? AND status = ?',
                        ('won', user_id, 'active')
                    )
                    conn.commit()
                    if cursor.rowcount == 0:
    ...
    def cancel_secret_room(self, user_id: int) -> dict:
        ...
        try:
            ...
            # Атомарно меняем статус ДО возврата ставки
            conn = self.get_connection()
            cursor = conn.cursor()
            cursor.execute(
                'UPDATE secret_room_games SET status = ? WHERE user_id = ? AND status = ?',
    ...
    def cleanup_old_games(self):
        ...
        try:
            ...
            # Атомарно помечаем старые активные игры как 'timeout',
            # чтобы не обработать их дважды при параллельном запуске
            cursor.execute(
                'UPDATE mines_games SET status = ? WHERE created_at < ? AND status = ?',
                ('timeout', timeout_ago, 'active')
            )
            cursor.execute(
                'UPDATE box_games SET status = ? WHERE created_at < ? AND status = ?',
                ('timeout', timeout_ago, 'active')
            )
            cursor.execute(
                'UPDATE secret_room_games SET status = ? WHERE created_at < ? AND status = ?',
                ('timeout', timeout_ago, 'active')
            )
            conn.commit()
            
            # Мины
            cursor.execute('SELECT * FROM mines_games WHERE status = ?', ('timeout',))
            ...
            cursor.execute('DELETE FROM mines_games WHERE status = ?', ('timeout',))
            
            # Шкатулки
            cursor.execute('SELECT * FROM box_games WHERE status = ?', ('timeout',))
            ...
            cursor.execute('DELETE FROM box_games WHERE status = ?', ('timeout',))
            
            # Тайная комната
            cursor.execute('SELECT * FROM secret_room_games WHERE status = ?', ('timeout',))
            ...
            cursor.execute('DELETE FROM secret_room_games WHERE status = ?', ('timeout',))
...
async def cancel_box_game(callback: CallbackQuery):
    ...
    async with lock:
        ...
        final_balance = new_balance if success else db.get_balance(user_id)
        await callback.message.edit_text(
            f"{user_link} отменил игру\n"
            f"💰 Ставка {format_number(game['bet'])} GRAM возвращена\n"
            f"💵 Новый баланс: {format_number(final_balance)} GRAM",
            parse_mode="HTML"
        )
        await callback.answer("✅ Игра отменена, ставка возвращена", show_alert=False)
...
@dp.callback_query(F.data.startswith("confirm_reset_") & ~F.data.startswith("confirm_reset_new_"))
