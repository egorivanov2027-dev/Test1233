...
class SQLiteDatabase:
    ...
    def start_mines_game(self, user_id: int, chat_id: int, bet, mines_positions: List[int]) -> bool:
        ...
        try:
            ...
            cursor.execute('''
                INSERT INTO mines_games (user_id, chat_id, bet, current_win, mines_positions, status, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (user_id, chat_id, bet_str, bet_str, json.dumps(mines_positions), 'active', now))
            conn.commit()
