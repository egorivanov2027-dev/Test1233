...
class SQLiteDatabase:
    ...
    def get_balance(self, user_id: int) -> str:
        for attempt in range(3):
            try:
                ...
                conn.close()
                return str(result['balance']) if result else '0'
            except sqlite3.OperationalError as e:
                if "locked" in str(e) and attempt < 2:
                    time.sleep(0.1)
                    continue
    ...
    def update_balance(self, user_id: int, amount) -> str:
        for attempt in range(3):
            try:
                conn = self.get_connection()
                cursor = conn.cursor()
                ...
                current_balance = str(current['balance']) if current else '0'
                ...
                if "locked" in str(e) and attempt < 2:
                    ...
                    continue
                logger.error(f"Ошибка БД: {e}")
                return str(amount) if attempt == 2 else '0'
        return '0'

    def reset_balance(self, user_id: int) -> str:
        for attempt in range(3):
            try:
                conn = self.get_connection()
                cursor = conn.cursor()
                cursor.execute('SELECT balance FROM users WHERE user_id = ?', (user_id,))
                result = cursor.fetchone()
                old_balance = str(result['balance']) if result else '0'
    ...
    def get_top_users(self, limit: int = 50) -> List[dict]:
        all_users = self.get_all_users()
        all_users.sort(key=lambda x: len(str(x['balance'])), reverse=True)
    ...
    def give_bonus(self, user_id: int):
        for attempt in range(3):
            try:
                conn = self.get_connection()
                cursor = conn.cursor()
    ...
    def give_start_bonus(self, user_id: int):
        for attempt in range(3):
            try:
                conn = self.get_connection()
                cursor = conn.cursor()
    ...
    def transfer(self, from_id: int, to_id: int, amount, chat_id: int = None):
        for attempt in range(3):
            try:
                conn = self.get_connection()
                cursor = conn.cursor()
                ...
                try:
                    ...
                    conn.close()
                    raise ValueError("❌ Ошибка при проверке баланса")
                ...
                try:
                    ...
                    conn.close()
                    raise ValueError(f"❌ Ошибка при вычислении баланса: {e}")
                ...
                if "locked" in str(e) and attempt < 2:
                    ...
                    continue
                logger.error(f"Ошибка БД: {e}")
                raise ValueError("❌ Ошибка базы данных, попробуйте позже")
    ...
    def add_transaction(self, transaction: dict):
        for attempt in range(3):
            try:
                ...
                cursor = conn.cursor()
                cursor.execute('''
                    INSERT INTO transactions (from_id, to_id, amount, type, chat_id, comment)
                    VALUES (?, ?, ?, ?, ?, ?)
                ''', (
    ...
    def create_promocode(self, code: str, amount: str, max_uses: int = 1) -> dict:
        code = code.upper()
        for attempt in range(3):
            try:
                conn = self.get_connection()
    ...
    def start_box_game(self, user_id: int, chat_id: int, bet) -> bool:
        try:
            ...
            cursor.execute('''
                INSERT INTO box_games (user_id, chat_id, bet, gold_box, opened_boxes)
                VALUES (?, ?, ?, ?, ?)
            ''', (user_id, chat_id, bet_str, gold_box, json.dumps([])))
    ...
    def open_multiple_cases(self, user_id: int, case_type: str, count: int) -> dict:
        try:
            ...
            self.update_balance(user_id, '-' + total_price)
    ...
    def sell_skin(self, user_id: int, skin_id: int) -> dict:
        try:
            ...
            self.update_balance(user_id, skin['price'])
    ...
    def sell_all_skins(self, user_id: int) -> dict:
        try:
            ...
            self.update_balance(user_id, str(total_amount))
    ...
    def start_mines_game(self, user_id: int, chat_id: int, bet, mines_positions: List[int]) -> bool:
        try:
            ...
            cursor.execute('''
                INSERT INTO mines_games (user_id, chat_id, bet, current_win, mines_positions)
                VALUES (?, ?, ?, ?, ?)
            ''', (user_id, chat_id, bet_str, bet_str, json.dumps(mines_positions)))
            conn.commit()
            conn.close()
    ...
    def open_cell(self, user_id: int, cell: int) -> dict:
        try:
            ...
            if len(game['opened_cells']) == len(safe_cells):
                self.update_balance(user_id, new_win)
                self.add_transaction({
                    'to_id': user_id,
                    'amount': new_win,
                    'type': 'mines_win'
    ...
    def take_win(self, user_id: int) -> dict:
        try:
            game = self.get_mines_game(user_id)
            if not game:
                return {'status': 'error', 'message': 'Нет активной игры'}
            ...
            self.end_mines_game(user_id)
            
            logger.info(f"💰 Игрок {user_id} забрал выигрыш {win_amount}")
            
            # Возвращаем данные для отображения мин
            return {
                'status': 'taken',
                'amount': win_amount,
    ...
    def start_secret_room(self, user_id: int, chat_id: int, bet: str) -> bool:
        try:
            ...
            cursor.execute('''
                ...
                (user_id, chat_id, bet, current_win, treasure_doors, monster_doors, empty_doors)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (user_id, chat_id, bet, bet, 
                  json.dumps(treasure_doors), json.dumps(monster_doors), json.dumps(empty_doors)))
            conn.commit()
            conn.close()
    ...
    def open_secret_door(self, user_id: int, door: int) -> dict:
        ...
        if door in game['monster_doors']:
            ...
            if game['treasures_found'] == 3:
                self.update_balance(user_id, game['current_win'])
                self.add_transaction({
                    'to_id': user_id,
    ...
    def cancel_secret_room(self, user_id: int) -> dict:
        ...
        self.update_balance(user_id, game['bet'])
        self.add_transaction({
            'to_id': user_id,
            'amount': game['bet'],
    ...
    def cleanup_old_games(self):
        """Удаляет игры старше 2 минут и возвращает ставки"""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            ...
            # Мины
            cursor.execute('SELECT * FROM mines_games WHERE created_at < ?', (timeout_ago,))
            ...
            for game in old_mines:
                ...
                logger.info(f"⏰ Игра в мины пользователя {game['user_id']} удалена")
            
            cursor.execute('DELETE FROM mines_games WHERE created_at < ?', (timeout_ago,))
            
            # Шкатулки
            cursor.execute('SELECT * FROM box_games WHERE created_at < ?', (timeout_ago,))
            ...
            for game in old_boxes:
                ...
                logger.info(f"⏰ Игра в шкатулки пользователя {game['user_id']} удалена")
            
            cursor.execute('DELETE FROM box_games WHERE created_at < ?', (timeout_ago,))
            
            # Тайная комната
            cursor.execute('SELECT * FROM secret_room_games WHERE created_at < ?', (timeout_ago,))
            ...
            for game in old_secret:
                ...
                logger.info(f"⏰ Игра в тайную комнату пользователя {game['user_id']} удалена")
            
            cursor.execute('DELETE FROM secret_room_games WHERE created_at < ?', (timeout_ago,))
            
            conn.commit()
            conn.close()
            
            return len(old_mines) + len(old_boxes) + len(old_secret)
        except Exception as e:
...
def get_mines_game_keyboard(opened_cells: List[int] = None, game_over: bool = False, show_mines: List[int] = None):
    ...
    # Кнопки отмены и забора выигрыша показываем только если игра активна
    if not game_over:
...
async def confirm_mass_case(callback: CallbackQuery):
    ...
    if result['status'] == 'success':
        ...
        text += f"📊 Прибыль: {format_number(str(int(Decimal(result['total_value']) - Decimal(result['total_price']))))} GRAM\n\n"
        text += "📋 Лучшие скины:\n"
        ...
        text += f"\n💰 Новый баланс: {format_number(new_balance)} GRAM"
...
async def sell_all_skins(callback: CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Да, продать всё", callback_data="confirm_sell_all")
...
async def confirm_sell_all(callback: CallbackQuery):
    user_id = callback.from_user.id
...
async def chat_transfer_reply(message: Message):
    ...
    try:
        db.transfer(from_user.id, to_user.id, amount, message.chat.id)
...
async def chat_transfer_username(message: Message):
    ...
    try:
        db.transfer(from_user.id, to_id, amount, message.chat.id)
        from_link = f"<a href='tg://user?id={from_user.id}'>{from_user.first_name}</a>"
...
async def mines_start(message: Message):
    ...
    if parts[1] == "фулл":
        ...
        try:
            if Decimal(bet) < 5:
                await message.answer("❌ Недостаточно средств для минимальной ставки (5 GRAM)")
                return
        except:
        ...
        try:
            bet = parts[1]
            if Decimal(bet) < 5:
                await message.answer("❌ Минимальная ставка: 5 GRAM")
                return
        except:
            await message.answer("❌ Укажите корректную сумму или 'фулл'")
            return

    try:
        balance = db.get_balance(user.id)
        if Decimal(balance) < Decimal(bet):
...
async def secret_room_start(message: Message):
    ...
    if parts[1] == "фулл":
        ...
        try:
            if Decimal(bet) < 100:
                await message.answer("❌ Недостаточно средств для минимальной ставки (100 GRAM)")
                return
        except:
            await message.answer("❌ Ошибка при проверке баланса")
            return
        ...
        try:
            bet = parts[1]
            if Decimal(bet) < 100:
                await message.answer("❌ Минимальная ставка: 100 GRAM")
                return
        except:
            await message.answer("❌ Укажите корректную сумму или 'фулл'")
            return
...
async def admin_process_promo_uses(message: Message, state: FSMContext):
    ...
    db.create_promocode(code, amount, uses)
